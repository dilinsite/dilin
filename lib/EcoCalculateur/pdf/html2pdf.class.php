...
*/
		...
		* @return	null
		*/	
		protected function Image($src, $sub_li=false, $is_dynamique_image=false)
		{
			// est-ce que c'est une image ?
			$savedSRC = $src;
			...
			if (!$this->sub_part && !$this->isSubPart)
			{
				if ($is_dynamique_image)
				{
					$savedSRC = str_replace("&amp;", "&", $savedSRC);
					$this->pdf->Image($savedSRC, $x, $y, $w, $h, '', $this->inLink);
				...
				{
					$this->pdf->setFillColorArray(array(255, 0, 0));
...
*/			
		...
		*/	
		protected function o_IMG($param)
		{
			...
			//	On regarde si l'image est une variable global PHP.
			if (strpos($src, 'IMAGE_DYNAMIQUE:') !== false)
			{
				$globalVarName = substr($src, 16);
				$res = $this->Image($globalVarName, isset($param['sub_li']), true);
			}
			else
			{
